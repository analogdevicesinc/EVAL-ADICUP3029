...
public:
	/*!
	     ...
	     */
	typedef enum {
		/*!< ID register does not match the expected value. */
		ERROR_CODE_ID_MISMATCH = 0u,
		/*!< Timed out waiting for a conversion to complete. */
		ERROR_CODE_RDY_TIMEOUT = 1u,
	} ERROR_CODES;
...
public:
	 ...
	 *
	 * @return SENSOR_RESULT. This value can be set using #SET_SENSOR_ERROR
	 * 		   (type, error).
	 *
	 ...
	 * @note    An assumption is made that the SPI bus specified in the
	 * 			configuration file will not be modified by another sensor.
	 * 			As a result, it will only be configured during this function
	 * 			and not on a transaction by transaction basis.
	 */
	 ...
	 *          on chip register. This API has to be implemented by all ADC
	 *          classes.
	 */
	 ...
	 * @param  [in] size       : The number of bytes to read. The largest
	 * 							 register size is 16 bits so this needs to be
	 * 							 less than 3 bytes.
	 ...
	 *
	 */
	virtual SENSOR_RESULT   readRegister(uint32_t regAddress,
										 uint32_t size,
										 uint8_t * regValue);
	 ...
	 * @param  [in] size       : The number of bytes to write. The largest
	 * 							 register size is 16 bits so this needs to be
	 * 							 less than 3 bytes.
	 *
	 * @return SENSOR_RESULT. This value can be set using #SET_SENSOR_ERROR
	 ...
	 *
	 */
	virtual SENSOR_RESULT  writeRegister(uint32_t regAddress,
										 uint32_t size,
										 uint32_t regValue);
	...
	/*! Reads the value of the specified register without a device state
	 *  check. */
	int32_t NoCheckReadRegister(ad7124_st_reg* pReg);

	/*! Writes the value of the specified register without a device state
	 *  check. */
